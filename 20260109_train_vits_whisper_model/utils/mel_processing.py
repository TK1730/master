import warnings

import torch
import torchaudio.transforms as T

spectrogram_basis = {}
mel_scale_basis = {}
mel_spectrogram_basis = {}

# warnings.simplefilter(action='ignore', category=FutureWarning)
warnings.filterwarnings(action="ignore")
MAX_WAV_VALUE = 32768.0


def dynamic_range_compression_torch(x, C=1, clip_val=1e-5):
    """
    PARAMS
    ------
    C: compression factor
    """
    return torch.log(torch.clamp(x, min=clip_val) * C)


def dynamic_range_decompression_torch(x, C=1):
    """
    PARAMS
    ------
    C: compression factor used to compress
    """
    return torch.exp(x) / C


def spectral_normalize_torch(magnitudes):
    output = dynamic_range_compression_torch(magnitudes)
    return output


def spectral_de_normalize_torch(magnitudes):
    output = dynamic_range_decompression_torch(magnitudes)
    return output


def wav_to_spec(
        y: torch.Tensor,
        n_fft,
        sample_rate,
        hop_length,
        win_length,
        center=False
) -> torch.Tensor:
    assert torch.min(y) >= -1.0, f"min value is {torch.min(y)}"
    assert torch.max(y) <= 1.0, f"max value is {torch.max(y)}"

    global spectrogram_basis
    dtype_device = str(y.dtype) + "_" + str(y.device)
    hparams = dtype_device + "_" + str(n_fft) + "_" + str(hop_length)
    if hparams not in spectrogram_basis:
        spectrogram_basis[hparams] = T.Spectrogram(
            n_fft=n_fft,
            win_length=win_length,
            hop_length=hop_length,
            pad=(n_fft - hop_length) // 2,
            power=1,
            center=center,
        ).to(device=y.device, dtype=y.dtype)

    spec = spectrogram_basis[hparams](y)
    spec = torch.sqrt(spec.pow(2) + 1e-6)
    return spec


def spec_to_mel(
        spec: torch.Tensor,
        n_fft, n_mels,
        sample_rate,
        f_min,
        f_max,
        norm=True
) -> torch.Tensor:
    global mel_scale_basis
    dtype_device = str(spec.dtype) + "_" + str(spec.device)
    hparams = (dtype_device + "_"
               + str(n_fft) + "_"
               + str(n_mels) + "_"
               + str(f_max))

    if hparams not in mel_scale_basis:
        mel_scale_basis[hparams] = T.MelScale(
            n_mels=n_mels,
            sample_rate=sample_rate,
            f_min=f_min,
            f_max=f_max,
            n_stft=n_fft // 2 + 1,
            norm="slaney",
            mel_scale="slaney"
        ).to(device=spec.device, dtype=spec.dtype)

    mel = torch.matmul(mel_scale_basis[hparams].fb.T, spec)
    if norm:
        mel = spectral_normalize_torch(mel)
    return mel


def wav_to_mel(
        y: torch.Tensor,
        n_fft, num_mels,
        sampling_rate,
        hop_size,
        win_size,
        fmin,
        fmax,
        center=False,
        norm=True
) -> torch.Tensor:
    assert torch.min(y) >= -1.0, f"min value is {torch.min(y)}"
    assert torch.max(y) <= 1.0, f"max value is {torch.max(y)}"

    global mel_spectrogram_basis
    dtype_device = str(y.dtype) + "_" + str(y.device)
    hparams = (
        dtype_device + "_"
        + str(n_fft) + "_"
        + str(num_mels) + "_"
        + str(hop_size) + "_"
        + str(fmax)
    )
    if hparams not in mel_spectrogram_basis:
        mel_spectrogram_basis[hparams] = T.MelSpectrogram(
            sample_rate=sampling_rate,
            n_fft=n_fft,
            win_length=win_size,
            hop_length=hop_size,
            n_mels=num_mels,
            f_min=fmin,
            f_max=fmax,
            pad=(n_fft - hop_size) // 2,
            power=1,
            center=center,
            norm="slaney",
            mel_scale="slaney",
        ).to(device=y.device, dtype=y.dtype)

    mel = mel_spectrogram_basis[hparams](y)
    if norm:
        mel = spectral_normalize_torch(mel)
    return mel


if __name__ == "__main__":
    import librosa
    wav = "preprocessed/jvs_ver1/jvs001/nonpara30w/wav24kHz16bit/BASIC5000_0025.wav"

    y, sr = librosa.load(wav, sr=None)
    mel = wav_to_mel(
        torch.from_numpy(y).unsqueeze(0),
        n_fft=1024,
        num_mels=80,
        sampling_rate=sr,
        hop_size=256,
        win_size=1024,
        fmin=0,
        fmax=8000,
        center=False,
        norm=True,
    )

    import matplotlib.pyplot as plt
    import numpy as np

    # Tensor -> numpy
    if isinstance(mel, torch.Tensor):
        mel_np = mel.detach().cpu().numpy()
    else:
        mel_np = np.array(mel)

    # shape 調整: (n_mels, time) を想定
    if mel_np.ndim == 3:
        # 例: (1, n_mels, time) -> (n_mels, time)
        mel_np = mel_np.squeeze(0)
    if mel_np.ndim == 2:
        img = mel_np
    else:
        # 最後の手段で最初の2次元を使う
        img = mel_np.reshape(mel_np.shape[0], -1)
    print(f"mel.max(): {mel_np.max():.3f}, mel.min(): {mel_np.min():.3f}")
    plt.figure(figsize=(10, 4))
    plt.imshow(img, aspect="auto", origin="lower", interpolation="nearest", cmap="magma")
    plt.colorbar(format="%.2f")
    plt.xlabel("Time frames")
    plt.ylabel("Mel bins")
    plt.title("Mel spectrogram")
    plt.tight_layout()
    plt.savefig("mel.png", dpi=150)
    plt.show()

