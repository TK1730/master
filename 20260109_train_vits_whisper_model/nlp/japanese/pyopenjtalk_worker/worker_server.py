...
class WorkerServer:
    ...
    def handle_request(self, request: dict[str, Any]) -> dict[str, Any]:
        """クライアントから受け取ったリクエストを解析して、処理を実行し結果を返す

        Args:
            request (dict[str, Any]): クライアントから受け取ったリクエスト
    ...
    def start_server(self, port: int, no_client_timeout: int = 30) -> None:
        """ワーカサーバーを起動し、クライアントからの接続とリクエストを待ち受け、
           処理する

        Args:
            port (int): ワーカプロセスが通信を待ち受けるネットワークポート
            no_client_timeout (int, optional): クライアントからの接続がない場合に
            サーバーを終了するまでのタイムアウト時間(秒)。デフォルトは30秒。
        """
