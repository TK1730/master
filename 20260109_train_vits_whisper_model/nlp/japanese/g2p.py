...
from nlp.japanese.mora_list import MORA_KATA_TO_MORA_PHONEMES, VOWELS
...
def g2p(
    ...
    Args:
        ...
        use_jp_extra (bool, optional): False の場合、「ん」の音素を「N」ではなく「n」とする。
        Defaults to True.
        raise_yomi_error (bool, optional): False の場合、読めない文字が「'」として発音される。
        Defaults to False.
    ...
    # 読めない文字は raise_yomi_error=True なら例外、False なら読めない文字を「'」として返ってくる
    ...
    for i in sep_text:
        if i not in PUNCTUATIONS:
            sep_tokenized.append(list(i))  # ここでおそらく`i`が文字単位に分割される
        else:
...
def adjust_word2ph(
    ...
    # ## word2ph は正規化された読み上げテキストの文字数に対応しているので、要素数はそのまま given_phone
    # ## で増減した音素数に合わせて各要素の値を増減する
    for word2ph_element_index, word2ph_element in enumerate(word2ph):
        ...
        for _ in range(word2ph_element):
            ...
            if current_diff is not None:
                ...
                diff_in_phonemes = \
                    (len(current_diff["given"]["value"])
                     - len(current_diff["generated"]["value"]))  # fmt: skip
    ...
    assert len(given_phone) == sum(
        adjusted_word2ph
    ), f"{len(given_phone)} != {sum(adjusted_word2ph)}"  # fmt: skip

    # generated_phone から given_phone の間で音素が減った場合
    # (例: a, sh, i, t, a -> a, s, u) 、
    ...
    for adjusted_word2ph_element_index, adjusted_word2ph_element in enumerate(
            adjusted_word2ph):  # fmt: skip
        # もし現在の要素が 1 未満ならば
    ...
    # 逆に、generated_phone から given_phone の間で音素が増えた場合
    # (例: a, s, u -> a, sh, i, t, a) 、
...
def __g2phone_tone_wo_punct(text: str) -> list[tuple[str, int]]:
    ...
    例: "こんにちは、世界ー。。元気？！" →
    [('k', 0), ('o', 0), ('N', 1), ('n', 1), ('i', 1), ('ch', 1), ('i', 1),
    ('w', 1), ('a', 1), ('s', 1), ('e', 1), ('k', 0), ('a', 0), ('i', 0),
    ('i', 0), ('g', 1), ('e', 1), ('N', 0), ('k', 0), ('i', 0)]
...
def __pyopenjtalk_g2p_prosody(
    ...
    sequence-to-sequence acoustic modeling for neural TTS`_ with some r9y9's
    tweaks.
    ...
    .. _`Prosodic features control by symbols as input of sequence-to-sequence
    ...
    for n in range(N):
        ...
        # current phoneme
        p3 = __PYOPENJTALK_G2P_PROSODY_P3_PATTERN.search(lab_curr).group(
            1
        )  # type: ignore
...
def __kata_to_phoneme_list(text: str) -> list[str]:
    ...
    def long_replacement(match: re.Match) -> str:
        first = match.group(1)
        bars = match.group(2)
        return first + (" " + first) * len(bars)

    spaced_phonemes = __MORA_PATTERN.sub(_mora_sub, text)
...
def __align_tones(
    ...
    phones_with_punct:
        ...
        "o", "o", "o", "m", "o", "u", "."]
    phone_tone_list:
        [("w", 0), ("a", 0), ("t", 1), ("a", 1), ("sh", 1), ("i", 1), ("w", 1),
        ("a", 1), ("_", 0), ("s", 0), ("o", 0), ("o", 1), ("o", 1), ("m", 1),
        ("o", 1), ("u", 0))]
    Return:
        [(".", 0), (".", 0), (".", 0), ("w", 0), ("a", 0), ("t", 1), ("a", 1),
        ("sh", 1), ("i", 1), ("w", 1), ("a", 1), (",", 0), (",", 0), ("s", 0),
        ("o", 0), ("o", 1), ("o", 1), ("m", 1), ("o", 1), ("u", 0), (".", 0)]

    Args:
        phones_with_punct (list[str]): punctuation を含む音素のリスト
        phone_tone_list (list[tuple[str, int]]): punctuation を含まない音素と
                                                 アクセントのペアのリスト
