...
from nlp.constants import Languages
...
def extract_bert_feature(
    ...
    if language == Languages.JP or language == Languages.EN or language == Languages.ZH:
        pass
    else:
...
def clean_text(
    ...
    Args:
        ...
        language (Languages): テキストの言語
        use_jp_extra (bool, optional): テキストが日本語の場合に JP-Extra
        モデルを利用するかどうか。Defaults to True.
        raise_yomi_error (bool, optional): False の場合、読めない文字が消えたような
        扱いとして処理される。Defaults to False.

    Returns:
        tuple[str, list[str], list[int], list[int]]: クリーニングされたテキストと、
        音素・アクセント・元のテキストの各文字に音素が何個割り当てられるかのリスト
    """
...
def clean_text_with_given_phone_tone(
    ...
    Args:
        text (str): クリーニングするテキスト
        language (Languages): テキストの言語
        given_phone (Optional[list[int]], optional): 読み上げテキストの読みを表す音素列。
        指定する場合は given_tone も別途指定が必要. Defaults to None.
        given_tone (Optional[list[int]], optional): アクセントのトーンのリスト.
        Defaults to None.
        use_jp_extra (bool, optional): テキストが日本語の場合に JP-Extra
        モデルを利用するかどうか。Defaults to True.
        raise_yomi_error (bool, optional): False の場合、読めない文字が消えたような扱いとして
        処理される。Defaults to False.

    Returns:
        tuple[str, list[str], list[int], list[int]]: クリーニングされたテキストと、音素・
        アクセント・元のテキストの各文字に音素が何個割り当てられるかのリスト
    """
    ...
    if given_phone is not None and given_tone is not None:
        ...
        if len(given_phone) != len(given_tone):
            raise InvalidPhoneError(
                f"Length of given_phone ({len(given_phone)}) != length "
                f"of given_tone ({len(given_tone)})"
            )
        ...
        if len(given_phone) != sum(word2ph):
            ...
            if language == Languages.JP:
                from nlp.japanese.g2p import adjust_word2ph
                ...
                # それでも一致しない場合、大半は読み上げテキストと given_phone
                # が著しく乖離していて調整し切れなかったことを意味する
                if len(given_phone) != sum(word2ph):
                    raise InvalidPhoneError(
                        f"Length of given_phone ({len(given_phone)}) != "
                        f"sum of word2ph ({sum(word2ph)})"
                    )
            else:
                raise InvalidPhoneError(
                    f"Length of given_phone ({len(given_phone)}) != "
        ...
        if len(phone) != len(given_tone):
            raise InvalidToneError(
                f"Length of phone ({len(phone)}) != length of given_tone"
                f"({len(given_tone)})"
            )
        tone = given_tone
...
def cleaned_text_to_sequence(
    ...
    Returns:
        tuple[list[int], list[int], list[int]]: List of integers corresponding
